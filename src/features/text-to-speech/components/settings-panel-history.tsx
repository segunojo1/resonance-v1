...
export function SettingsPanelHistory() {
  ...
  if (!generations.length) {
    return (
      <div className="flex h-full flex-col text-center items-center justify-center gap-2 p-8 ">
        ...
        <p className="max-w-43 text-center text-xs text-muted-foreground">
  ...
  return (
    <div className="flex flex-col gap-1 p-2">
      {generations.map((generation) => (
        <Link
          href={`/text-to-speech/history/${generation.id}`}
          key={generation.id}
